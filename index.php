    ...
    <?php foreach ($produtos as $produto): ?>
        <div class="card">
            <div class="card-image">
                <img src="<?= htmlspecialchars($produto->getImagemUrlExibicao()) ?>" alt="<?= htmlspecialchars($produto->getNome()) ?>">
            </div>
            <div class="card-body">
                <h2 class="card-title"><?= htmlspecialchars($produto->getNome()) ?></h2>
                <p class="card-price">R$ <?= number_format($produto->getPreco(), 2, ',', '.') ?></p>
                <div class="card-actions">
                    <a href="/sistema_vendas_tii09/pages/produtos/detalhes.php?id=<?= $produto->getId() ?>" class="btn btn-secondary">Detalhes</a>
                    <button class="btn btn-primary" onclick="adicionarAoCarrinho(
                        <?= $produto->getId() ?>,
                        '<?= htmlspecialchars(addslashes($produto->getNome())) ?>',
                        <?= $produto->getPreco() ?>,
                        '<?= htmlspecialchars($produto->getImagemUrlExibicao()) ?>'  
                    )">
